...
# ✅ Backbone: DINOv1 (ViT-S/16) + 轻量 Pyramid Adapter 输出 (1/4,1/8,1/16,1/32)
...
import sys
sys.path.insert(0, "/root/shared-nvme/dino-main")
...
# ============================ DINOv1 Backbone + Pyramid Adapter ============================ #

class DINOv1PyramidBackbone(nn.Module):
    """
    ...
    DINOv1 特点：
      - 官方 get_intermediate_layers(n) 只支持“最后 n 层”
      - 不支持像 DINOv3 那样直接传 [2,5,8,11]
      - 返回的是 token 序列，需要手动 reshape 到 [B,C,H/16,W/16]

    这里改为：
      - 手动遍历 blocks
      - 按 out_indices 抽取指定层（默认 [2,5,8,11]）
      - 在 1/16 融合 => f16
      - 再通过 up/down 生成 f8, f4, f32
    """
    def __init__(
        self,
        arch="dino_vits16",
        ckpt_path="/root/shared-nvme/pretrain/dino_deitsmall16_pretrain.pth",
        out_indices=None,
        ...
        # ---- 强制从 dino-main 加载 utils.py 和 vision_transformer.py，避免和当前工程里的 utils 冲突 ----
        try:
            import importlib.util
            import sys as _sys

            dino_repo = "/root/shared-nvme/dino-main"
            vt_path = os.path.join(dino_repo, "vision_transformer.py")
            utils_path = os.path.join(dino_repo, "utils.py")

            if not os.path.isfile(vt_path):
                raise FileNotFoundError(f"vision_transformer.py not found: {vt_path}")
            if not os.path.isfile(utils_path):
                raise FileNotFoundError(f"utils.py not found: {utils_path}")

            # 先把 dino-main 的 utils 注册成模块名 "utils"
            ...
            _sys.modules["utils"] = mod_utils

            # 再加载 vision_transformer
            spec_vt = importlib.util.spec_from_file_location("dino_v1_vision_transformer", vt_path)
            vits = importlib.util.module_from_spec(spec_vt)
            spec_vt.loader.exec_module(vits)

        except Exception as e:
            raise ImportError(f"Cannot import DINOv1 vision_transformer.py from dino-main. err={e}")

        arch_l = self.arch.lower()
        if arch_l in ["dino_vits16", "vit_small", "vit_small_16", "vits16"]:
            self.backbone = vits.vit_small(patch_size=16, num_classes=0)
        elif arch_l in ["dino_vitb16", "vit_base", "vit_base_16", "vitb16"]:
            self.backbone = vits.vit_base(patch_size=16, num_classes=0)
        else:
            raise ValueError(
                f"[DINOv1] Unsupported arch='{self.arch}'. "
                f"Use one of ['dino_vits16', 'vit_small', 'dino_vitb16', 'vit_base']"
            )

        self.embed_dim = int(getattr(self.backbone, "embed_dim", getattr(self.backbone, "num_features", 384)))
        ...
        self.depth = int(len(getattr(self.backbone, "blocks", [])))
        if self.depth <= 0:
            raise RuntimeError("[DINOv1] backbone.blocks is empty.")
        ...
        # 安全裁剪到合法 block 范围
        self.out_indices = [i for i in self.out_indices if 0 <= int(i) < self.depth]
        if len(self.out_indices) == 0:
            raise ValueError(f"[DINOv1] invalid out_indices={out_indices} for depth={self.depth}")

        if self.fuse == "cat":
            self.fuse_proj = nn.Sequential(
                nn.Conv2d(self.embed_dim * len(self.out_indices), self.embed_dim, 1, bias=False),
    ...
    def _load_ckpt(self, ckpt_path):
        if ckpt_path is None or not os.path.isfile(ckpt_path):
            print(f"[DINOv1] ckpt not found: {ckpt_path}. Use random init.")
            return
        ...
        if isinstance(raw, (dict, OrderedDict)):
            ...
            print(f"[DINOv1] Unrecognized ckpt type: {type(raw)}. Use random init.")
            return
        ...
        print(
            f"[DINOv1] loaded: {ckpt_path} | "
            f"matched={len(filtered)} | "
            f"missing={len(incompatible.missing_keys)} | "
            f"unexpected={len(incompatible.unexpected_keys)}"
        )

    def _tokens_to_map(self, x_tokens, H, W):
        """
        x_tokens: [B, 1+N, C] or [B, N, C]
        return:   [B, C, H/P, W/P]
        """
        if x_tokens.dim() != 3:
            raise RuntimeError(f"[DINOv1] unexpected token shape: {x_tokens.shape}")

        B, N_all, C = x_tokens.shape
        ...
        if N_all == Hp * Wp + 1:
            ...
            # 回退：如果输入不是标准整除，尽量按 N 推断
            if N_all > 1:
                maybe = N_all - 1
                if Hp * Wp == maybe:
                    x_tokens = x_tokens[:, 1:, :]
                else:
                    # 强行去 cls 再按 sqrt(N) 兜底
                    x_tokens = x_tokens[:, 1:, :]
                    N = x_tokens.shape[1]
                    side = int(round(math.sqrt(N)))
                    if side * side != N:
                        raise RuntimeError(
                            f"[DINOv1] cannot reshape tokens to feature map: "
                            f"N_all={N_all}, H={H}, W={W}, patch={self.patch_size}"
                        )
                    Hp = side
                    Wp = side
        ...
        if N != Hp * Wp:
            raise RuntimeError(
                f"[DINOv1] token number mismatch after cls removal: "
                f"N={N}, Hp={Hp}, Wp={Wp}, H={H}, W={W}, patch={self.patch_size}"
            )
    ...
    def _get_intermediate(self, x):
        ...
        for i, blk in enumerate(self.backbone.blocks):
            ...
            if i in self.out_indices:
                feat_tokens = self.backbone.norm(x)
                feat_map = self._tokens_to_map(feat_tokens, H, W)
    ...
    def forward(self, x):
        ...
        if not isinstance(feats, (list, tuple)) or len(feats) == 0:
            raise RuntimeError(f"[DINOv1] _get_intermediate returned invalid feats: {type(feats)}")

        if len(feats) >= len(self.out_indices):
            feats4 = feats[-len(self.out_indices):]
        else:
            feats4 = list(feats) + [feats[-1]] * (len(self.out_indices) - len(feats))
...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        # ---- DINOv1 backbone ----
        # 优先读 dinov1_*；若 train 里还沿用 dinov3_* 配置名，也可回退兼容
        dinov1_ckpt = getattr(
            cfg, "dinov1_ckpt",
            getattr(cfg, "dinov3_ckpt", "/root/shared-nvme/pretrain/dino_deitsmall16_pretrain.pth")
        )
        dinov1_arch = getattr(
            cfg, "dinov1_arch",
            getattr(cfg, "dinov3_arch", "dino_vits16")
        )
        dinov1_fuse = getattr(
            cfg, "dinov1_fuse",
            getattr(cfg, "dinov3_fuse", "mean")
        )
        dinov1_freeze = bool(getattr(
            cfg, "dinov1_freeze",
            getattr(cfg, "dinov3_freeze", True)
        ))
        dinov1_fp16 = bool(getattr(
            cfg, "dinov1_fp16",
            getattr(cfg, "dinov3_fp16", False)
        ))
        dinov1_out_indices = getattr(cfg, "dinov1_out_indices", [2, 5, 8, 11])

        self.bkbone = DINOv1PyramidBackbone(
            arch=dinov1_arch,
            ckpt_path=dinov1_ckpt,
            out_indices=dinov1_out_indices,
            fuse=dinov1_fuse,
            freeze=dinov1_freeze,
            fp16_infer=dinov1_fp16,
        )
    ...
    def initialize(self):
        print("initialize net (DINOv1 backbone)")
