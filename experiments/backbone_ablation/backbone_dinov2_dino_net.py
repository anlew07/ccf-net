...
# ✅ Backbone: DINOv2 (ViT-S/14) + 轻量 Pyramid Adapter 输出 (相对 1/4,1/8,1/16,1/32)
...
import sys
sys.path.insert(0, "/root/shared-nvme/dinov2-main")
...
# ============================ DINOv2 Backbone + Pyramid Adapter ============================ #
...
class DINOv2PyramidBackbone(nn.Module):
    ...
    - 从 DINOv2 取 4 个中间层特征（reshape=True => [B,C,H/14,W/14]）
    - 融合得到主尺度 f_mid
    - 通过 up/down 生成 f8, f4, f32 风格的多尺度特征
    """
    def __init__(
        self,
        arch="dinov2_vits14",
        ckpt_path="/root/shared-nvme/pretrain/dinov2_vits14_pretrain.pth",
        out_indices=None,
        fuse="mean",
        freeze=True,
        fp16_infer=False,
        ...
        try:
            import dinov2.hub.backbones as bb
        except Exception as e:
            raise ImportError(f"Cannot import dinov2. Ensure dinov2-main is in PYTHONPATH. err={e}")

        available = sorted([n for n in dir(bb) if n.startswith("dinov2_")])
        if not hasattr(bb, self.arch):
            raise ValueError(
                f"[DINOv2] Unknown arch='{self.arch}'.\n"
                f"Available dinov2 backbones: {available}"
            )
    ...
    def _load_ckpt(self, ckpt_path):
        if ckpt_path is None or not os.path.isfile(ckpt_path):
            print(f"[DINOv2] ckpt not found: {ckpt_path}. Use random init.")
            return
        ...
        if isinstance(raw, (dict, OrderedDict)):
            ...
            print(f"[DINOv2] Unrecognized ckpt type: {type(raw)}. Use random init.")
            return
    ...
    def _pad_input(self, x):
        """
        将输入 pad 到 patch_size 的整数倍，避免 DINOv2 patch_embed 断言报错。
        只在右边和下边补零，尽量不破坏原图对齐。
        """
        B, C, H, W = x.shape
        ph = self.patch_size
    ...
    def _get_intermediate(self, x):
        ...
        if fn is None:
            raise AttributeError("[DINOv2] backbone has no get_intermediate_layers()")
    ...
    def forward(self, x):
        ...
        if not isinstance(feats, (list, tuple)) or len(feats) == 0:
            raise RuntimeError(f"[DINOv2] get_intermediate_layers returned invalid feats: {type(feats)}")

        feats4 = feats[-4:] if len(feats) >= 4 else (list(feats) + [feats[-1]] * (4 - len(feats)))

        if self.fuse == "cat":
            f_mid = self.fuse_proj(torch.cat(feats4, dim=1))
        else:
            f_mid = sum(feats4) / float(len(feats4))

        f8 = self.to_f8(F.interpolate(f_mid, scale_factor=2.0, mode="bilinear", align_corners=False))
        f4 = self.to_f4(F.interpolate(f_mid, scale_factor=4.0, mode="bilinear", align_corners=False))
        f32 = self.to_f32(F.avg_pool2d(f_mid, kernel_size=2, stride=2))
...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        # ---- DINOv2 backbone ----
        # 优先读 dinov2_*；若 train/config 还是旧字段，也兼容回退到 dinov3_*
        dinov2_ckpt = getattr(
            cfg, "dinov2_ckpt",
            getattr(cfg, "dinov3_ckpt", "/root/shared-nvme/pretrain/dinov2_vits14_pretrain.pth")
        )
        dinov2_arch = getattr(
            cfg, "dinov2_arch",
            getattr(cfg, "dinov3_arch", "dinov2_vits14")
        )
        dinov2_fuse = getattr(
            cfg, "dinov2_fuse",
            getattr(cfg, "dinov3_fuse", "mean")
        )
        dinov2_freeze = bool(getattr(
            cfg, "dinov2_freeze",
            getattr(cfg, "dinov3_freeze", True)
        ))
        dinov2_fp16 = bool(getattr(
            cfg, "dinov2_fp16",
            getattr(cfg, "dinov3_fp16", False)
        ))

        self.bkbone = DINOv2PyramidBackbone(
            arch=dinov2_arch,
            ckpt_path=dinov2_ckpt,
            fuse=dinov2_fuse,
            freeze=dinov2_freeze,
            fp16_infer=dinov2_fp16,
        )
        ...
        self.conv1 = nn.ModuleList([
            basicConv(ENC_DIM, 64, k=1, s=1, p=0),
            basicConv(ENC_DIM, 64, k=1, s=1, p=0),
            basicConv(ENC_DIM, 64, k=1, s=1, p=0),
            basicConv(ENC_DIM, 64, k=1, s=1, p=0),
            basicConv(ENC_DIM, 64, k=1, s=1, p=0),
    ...
    def initialize(self):
        print("initialize net (DINOv2 backbone)")
