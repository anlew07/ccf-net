...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        # ---- DINOv3 backbone ----
        dinov3_ckpt = getattr(cfg, "dinov3_ckpt",
                              "/root/shared-nvme/pretrain/dinov3_vits16_pretrain_lvd1689m-08c60483.pth")
        dinov3_arch = getattr(cfg, "dinov3_arch", "dinov3_vits16")
        dinov3_fuse = getattr(cfg, "dinov3_fuse", "mean")
    ...
    def forward(self, x, shape=None, epoch=None):
        ...
        dec_c3 = self.fusion[1](dec_c3_local, dec_c4_up)

        # ========== Stage-2 (1/8) - 使用cue但不用CoF ==========
        ...
        dec_c2 = dec_c2_base  # 不使用CoF

        # ========== Stage-1 (1/4) - 使用cue但不用CoF ==========
    ...
    def initialize(self):
        print("initialize net (DINOv3 backbone) - CP only, no CoF")
