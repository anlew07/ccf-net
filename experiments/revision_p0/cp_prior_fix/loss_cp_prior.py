...
def train_loss(image, mask, net, ctx,
    ...
    # ---------- P0 revision: sparse CP-prior supervision ---------- #
    # Keep the historical loss convention unchanged:
    # logit -> sigmoid -> [1-p, p] -> existing CrossEntropyLoss.
    ...
    if enc_sem_logit_raw is None or enc_edge_logit_raw is None:
        raise RuntimeError(
            ...
            "Use ccfnet_cp_prior.py with loss_cp_prior.py."
        )

    def cp_prior_out(logit):
        prob = torch.sigmoid(logit)
        return torch.cat((1 - prob, prob), dim=1)

    enc_sem_out = cp_prior_out(enc_sem_logit_raw)
    enc_edge_out = cp_prior_out(enc_edge_logit_raw)

    cp_sem_ce = (
        criterion(enc_sem_out, fg_label)
        + criterion(enc_sem_out, bg_label)
    )

    cp_edge_ce = (
        criterion(enc_edge_out, fg_label)
        + criterion(enc_edge_out, bg_label)
    )

    loss_cp_prior = (
        cp_prior_w * 0.5 * (cp_sem_ce + cp_edge_ce)
    )

    net.cp_prior_debug = {
        "sem_ce": cp_sem_ce.detach(),
        "edge_ce": cp_edge_ce.detach(),
    ...
    if sw is not None:
        ...
        _tb_add_scalars(sw, global_step, "loss", {
            ...
            "cp_prior_sem_ce": cp_sem_ce,
            "cp_prior_edge_ce": cp_edge_ce,
