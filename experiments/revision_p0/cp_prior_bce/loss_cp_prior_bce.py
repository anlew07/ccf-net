...
def train_loss(image, mask, net, ctx,
    ...
    # ---------- P0 revision v1b: balanced masked BCEWithLogits ---------- #
    # Only the NEW CP-prior auxiliary objective uses standard logit BCE.
    # The historical decoder/main losses remain completely unchanged.
    #
    # Scribble convention:
    #   1   = foreground
    #   0   = background
    ...
    #
    # FG and BG are averaged separately so sparse class imbalance does not
    # let the larger scribble class dominate the prior head.

    enc_sem_logit_raw = aux_cache_main.get(
        "enc_sem_logit_full_raw", None
    )
    enc_edge_logit_raw = aux_cache_main.get(
        "enc_edge_logit_full_raw", None
    )

    if enc_sem_logit_raw is None or enc_edge_logit_raw is None:
        raise RuntimeError(
            "Missing attached CP-prior logits. "
            "Use ccfnet_cp_prior_bce.py with loss_cp_prior_bce.py."
        )

    def balanced_scribble_bce(logit):
        z = logit.squeeze(1)
        ...
        if len(terms) == 0:
            # Keep graph connectivity but contribute zero if a pathological
            # batch contains no labeled scribble pixels.
            return z.sum() * 0.0

        return torch.stack(terms).mean()

    cp_sem_bce = balanced_scribble_bce(enc_sem_logit_raw)
    cp_edge_bce = balanced_scribble_bce(enc_edge_logit_raw)

    loss_cp_prior = (
        cp_prior_w * 0.5 * (cp_sem_bce + cp_edge_bce)
    )

    net.cp_prior_debug = {
        "sem_bce": cp_sem_bce.detach(),
        "edge_bce": cp_edge_bce.detach(),
        "total": loss_cp_prior.detach(),
    }
    ...
    loss2 = loss_struct2 + ce2 + reg2 + loss_cp_prior
    ...
    if sw is not None:
        ...
        _tb_add_scalars(sw, global_step, "loss", {
            ...
            "main_ce": ce2,
            "cp_prior_total": loss_cp_prior,
            "cp_prior_sem_bce": cp_sem_bce,
            "cp_prior_edge_bce": cp_edge_bce,
