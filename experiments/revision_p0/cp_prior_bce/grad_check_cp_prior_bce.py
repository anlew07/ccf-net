...
def main() -> None:
    ...
    prior_debug = getattr(net, "cp_prior_debug", {})
    if prior_debug:
        print(
            "cp_prior:",
            "sem_bce=%.8f" % float(prior_debug["sem_bce"].item()),
            "edge_bce=%.8f" % float(prior_debug["edge_bce"].item()),
            "weighted_total=%.8f" % float(prior_debug["total"].item()),
        )
