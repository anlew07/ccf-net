...
from A5_8_11_dino_net import Net
from tools import *
...
def cleanup_eval_checkpoints(save_dir: str, keep: int = 5):
    ...
    items.sort(key=lambda x: x[0])  # epoch 升序
...
def train(Dataset, Network, cfg, train_loss_fn, start_from=0):
    ...
    # parameter grouping：'bkbone' 归为 base
    base, head = [], []
    ...
    for epoch in range(start_from, cfg.epoch):
        ...
        while image is not None:
            ...
            # ✅ 修复：先 batch_idx += 1 再计算 niter（避免第一步 niter=-1）
            batch_idx += 1
            ...
            # loss = loss2 * 1 + loss3 * 0.6 + loss4 * 0.4 + loss5 * 0.2 + loss6 * 0.1
...
if __name__ == '__main__':
    ...
    EXP_NAME = f"5_8_11_dino_net_{run_time}"
    ...
    cfg = dataset.Config(
        datapath=f'{root}',
        savepath=f'./out_5_8_11_dino_net_/{EXP_NAME}/',
        mode='train',
        batch=16,
