...
# ✅ Backbone: DINOv3 (ViT-S/16) + 分支2直连金字塔输出 (1/4,1/8,1/16,1/32)
# ✅ 分支2：使用 DINOv3 的 2/5/8/11 四层，不融合，直接各层处理成后续输入
...
# ============================ DINOv3 Backbone + 分支2直连金字塔 ============================ #

class DINOv3PyramidBackbone(nn.Module):
    """
    分支2：
      使用 DINOv3 的 2/5/8/11 四层，不融合，直接各层处理成后续输入。
    ...
    def __init__(
        ...
        if len(self.out_indices) != 4:
            raise ValueError(
                f"[DINOv3][Branch-2] out_indices must contain 4 layers, got: {self.out_indices}"
            )
    ...
    def forward(self, x):
        ...
        if len(feats) != 4:
            raise RuntimeError(
                f"[DINOv3][Branch-2] expected 4 intermediate features, got {len(feats)}"
            )

        feat_2, feat_5, feat_8, feat_11 = feats

        f4  = self.to_f4(F.interpolate(feat_2,  scale_factor=4.0, mode="bilinear", align_corners=False))
        f8  = self.to_f8(F.interpolate(feat_5,  scale_factor=2.0, mode="bilinear", align_corners=False))
        f16 = self.to_f16(feat_8)
        f32 = self.to_f32(F.avg_pool2d(feat_11, kernel_size=2, stride=2))
...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        dinov3_fuse = getattr(cfg, "dinov3_fuse", "mean")  # 分支2保留接口但不实际使用
