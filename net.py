...
class ResNet(nn.Module):
    ...
    def initialize(self):
        self.load_state_dict(torch.load('./assets/resnet50-19c8e357.pth', weights_only=False), strict=False)
...
class Net(nn.Module):
    ...
    def initialize(self):
        ...
        if self.cfg.snapshot:
            self.load_state_dict(torch.load(self.cfg.snapshot, weights_only=False), strict=False)
        else:
            weight_init(self)